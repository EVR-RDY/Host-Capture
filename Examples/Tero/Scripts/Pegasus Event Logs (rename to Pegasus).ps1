...
#		$eventlogs = "Event Logs"
        ...
        & $EZ\EvtxExplorer\EvtxECmd.exe -d "$finalsave\Event Logs" --json "$finalsave\Event Logs"
		Remove-Item "$finalsave\Event Logs\*.evt"
    }
    if ((test-path -literalpath $single_collect_path\Evtx) -match "True") {
        & $EZ\EvtxExplorer\EvtxECmd.exe -d "$single_collect_path\Evtx" --json "$finalsave\Event Logs"
    }
}
...
function process {
	...
	foreach ($single_collect_path in $folders) {
		...
		if (test-path $single_collect_path\*.vhdx) {
			...
			& $drive\kape\kape.exe --msource "$vhdx_drive`:`\" --mdest "$finalsave" --module EvtxECmd --mef json
